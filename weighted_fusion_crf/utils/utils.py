...
conll_iob = {'B-BOOK': 0, 'I-BOOK': 1, 'B-OFI': 2, 'I-OFI': 3, 'B-PER': 4, 'I-PER': 5, 'O': 6}
...
def get_reader(file_path, gazetteer_path, max_instances=-1, max_length=128, target_vocab=None,
               encoder_model="ethanyt/guwenbert-large"):
...
def create_model(train_data, dev_data, tag_to_id, batch_size=64, dropout_rate=0.1, stage='fit', lr=1e-5,
                 encoder_model='ethanyt/guwenbert-large', num_gpus=1):
...
def get_trainer(gpus=4, is_test=False, out_dir=None, epochs=10):
    ...
    if torch.cuda.is_available():
        ...
        trainer = pl.Trainer(logger=logger, num_nodes=gpus, deterministic=False, max_epochs=epochs,
